...
namespace GleyTrafficSystem
{
    public class CidyMethods : Editor
    {
            ...
            for (int i = 0; i < nodes.Count; i++)
            {
                ...
                switch (node.type)
                {
                    case CiDyNode.IntersectionType.continuedSection:
                        ...
                        {
                            ExtractLaneConnectors(node.leftRoutes.routes[j].waypoints, lane.transform, j, i, maxSpeed, -1, vehicleTypes);
                        }
                        for (int j = 0; j < node.rightRoutes.routes.Count; j++)
                        {
                            ExtractLaneConnectors(node.rightRoutes.routes[j].waypoints, lane.transform, j, i, maxSpeed, -1, vehicleTypes);
                        }
                        break;
                    ...
                    case CiDyNode.IntersectionType.tConnect:
                        ...
                        for (int j = 0; j < node.intersectionRoutes.intersectionRoutes.Count; j++)
                        {
                            ...
                            ExtractLaneConnectors(node.intersectionRoutes.intersectionRoutes[j].route.waypoints, lane.transform, j, node.intersectionRoutes.intersectionRoutes[j].sequenceIndex, maxSpeed, allGleyIntersections.Count - 1, vehicleTypes);
                        }
                        break;
                }
        ...
        private static void ExtractLaneConnectors(List<Vector3> laneConnectors, Transform node, int laneIndex, int roadIndex, int speedLimit, int intersectionIndex, List<VehicleTypes> vehicleTypes)
        {
