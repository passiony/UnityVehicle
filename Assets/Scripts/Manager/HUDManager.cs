...
namespace Vertical
{
    ...
    {
        [Header("G1: 基础警报")] public GameObject steeringWheelIcon; // 红色方向盘图标
        public AudioSource voiceAlarm; // “请立即接管”语音
        private bool isFlashing;
        [Header("G2: 风险可视化")] public GameObject[] activeRiskZones;
        [Header("G3: 增强态势理解")] public GameObject[] safetyZones; // 绿色安全区域数组（左、中、右车道）

        public HUDLevel currentLevel = HUDLevel.G1;

        private void Start()
        {
            currentLevel = (HUDLevel)GlobalData.hudIndex;
        }
